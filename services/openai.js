...
class OpenAIService {
  ...
  async identifyRelevantLinks(links, baseUrl) {
    try {
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a research assistant helping job candidates learn about companies. Your task is to identify the most relevant pages that would help a candidate understand the company's business, culture, and work environment. Return a JSON object with categorized URLs."
          },
          {
            role: "user",
      ...
      const validLinks = parsedResponse.links.filter(link => 
        link && 
        typeof link === 'object' && 
  ...
  async analyzeCompany(scrapedContent, position) {
    try {
      const response = await this.openai.chat.completions.create({
        model: "gpt-4o-mini",
        messages: [
          {
            role: "system",
            content: "You are a company research assistant helping job candidates prepare for interviews. Provide very concise summaries (2-3 sentences per section) without using any markdown formatting. Focus on the most relevant information for the candidate's position."
          },
          {
            role: "user",
      ...
      // Clean up the response to ensure no markdown
